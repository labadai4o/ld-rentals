...
RegisterNetEvent('qb-rentals:client:openMenu', function(locationIndex)
	openNui(locationIndex)
end)

-- NUI callbacks
RegisterNUICallback('rentals:rent', function(data, cb)
	TriggerEvent('qb-rentals:client:tryRent', data)
	cb(true)
end)
...
RegisterNetEvent('qb-rentals:client:tryRent', function(data)
	-- Check if player already has an active rental
	if activeRental then
		SendNotification('Вече имате наето превозно средство. Не можете да наемете друго.', 'error')
		return
	end
	...
	if not spawnPoint then
		SendNotification('Няма свободно място за спаун.', 'error')
		return
	end

	-- ask server to charge and approve
	TriggerServerEvent('qb-rentals:server:rent', data.model, data.price, data.label, data.locationIndex, spawnPoint)
end)

RegisterNetEvent('qb-rentals:client:spawnApproved', function(model, label, spawnPoint, plate)
	local mdl = loadModel(model)
	if not mdl then
		SendNotification('Грешка при зареждане на модела.', 'error')
		return
	end
	local veh = CreateVehicle(mdl, spawnPoint.x, spawnPoint.y, spawnPoint.z, spawnPoint.w, true, false)
	if not DoesEntityExist(veh) then
		SendNotification('Неуспешно спаунване на превозното средство.', 'error')
		return
	end
	...
	SendNotification(('Наехте %s. Карайте внимателно!'):format(label), 'success')
	if nuiOpen then
		SetNuiFocus(false, false)
		SendNUIMessage({ action = 'close' })
		nuiOpen = false
...
RegisterNetEvent('qb-rentals:client:cleanupNPCs', function()
		...
		options = {
			{
				...
				action = function()
					...
					TriggerEvent('qb-rentals:client:openMenu', index)
				end
			}
		},
		distance = 2.0
...
CreateThread(function()
	...
	for i = 1, #Config.Locations do
		...
		AddTextComponentString(loc.label or (Config.Blip.label or 'Rentals'))
