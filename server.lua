...
RegisterNetEvent('ld-rentals:server:rent', function(model, price, label, locationIndex, spawnPoint)
	local src = source
	...
	if not valid then
		DropPlayer(src, _U('invalid_rental_data'))
		return
	end
	...
	if removeMoney(Player, price) then
		...
		TriggerClientEvent('ld-rentals:client:spawnApproved', src, model, label, spawnPoint, plate)
		SendNotification(src, _U('payment_success', price), 'success')
	else
		SendNotification(src, _U('insufficient_funds'), 'error')
	end
end)
...
AddEventHandler('onResourceStop', function(resourceName)
	if resourceName == GetCurrentResourceName() then
		...
		TriggerClientEvent('ld-rentals:client:cleanupNPCs', -1)
	end
end)
